...
// Fonction pour vérifier le token CSRF (non utilisé ici car affichage)
...
<body>
    ...
    <main>
        ...
        try {
            // Requête pour récupérer la liste des pays (on suppose id_pays et nom_pays)
            ...
            if ($statement->rowCount() > 0) {
                ...
                while ($row = $statement->fetch(PDO::FETCH_ASSOC)) {
                    // Utilisation de l'id_pays pour les actions
                    $id_pays = $row['id_pays']; 
                    
                    echo "<tr>";
                    echo "<td>" . htmlspecialchars($row['nom_pays'], ENT_QUOTES, 'UTF-8') . "</td>";
                    echo "<td><button onclick='openModifyCountryForm($id_pays)'>Modifier</button></td>";
                    echo "<td><button onclick='deleteCountryConfirmation($id_pays)'>Supprimer</button></td>";
                    echo "</tr>";
                }
            ...
            echo "Erreur : Impossible de charger la liste des pays. " . htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8');
        }
        ?>
    ...
    <script>
        function openAddCountryForm() {
            // Pointez vers la page d'ajout d'un pays
            window.location.href = 'add-country.php';
        }

        function openModifyCountryForm(id_pays) {
            // Pointez vers la page de modification en passant l'ID du pays
        ...
        function deleteCountryConfirmation(id_pays) {
            ...
            if (confirm("Êtes-vous sûr de vouloir supprimer ce pays? Cela affectera potentiellement les athlètes qui lui sont rattachés.")) {
